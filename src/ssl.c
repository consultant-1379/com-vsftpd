...
static void ipStructToStr(ASN1_OCTET_STRING *iPAddress, struct mystr* p_str);
static void extractSubjectAltName(X509* p_cert, struct mystr* p_str);
...
ssl_init(struct vsf_session* p_sess)
{
  if (!ssl_inited)
  {
    ...
    /* During the uplift of com-vsftpd from version vsftpd 3.0.3 to version vsftpd 3.0.4 changes the below code was commented
       This is because TLS 1.3 protocol requires Openssl 1.1.1 and currently Openssl version 1.0.2p is used
       When the Openssl version is uplifted to Openssl 1.1.1 the below code should be uncommented to enable TLS version 1.3 protocol */
 /*   if (!tunable_tlsv1_3)
    {
...
static void
ipStructToStr(ASN1_OCTET_STRING *iPAddress, struct mystr* p_str)
{
    char data[256];
    memset(data, 0, 256);

    unsigned char *userName = iPAddress->data;
    if(iPAddress->length == 4) {
        // IPv4
        snprintf(data, sizeof(data),
                          "%d.%d.%d.%d", userName[0], userName[1], userName[2], userName[3]);
        str_alloc_text(p_str, data);
    } else if(iPAddress->length == 16) {
        //IPv6
        int i;
        for (i = 0; i < 8; i++) {
            snprintf(data, sizeof(data),
                     "%X", userName[0] << 8 | userName[1]);
            userName += 2;
            str_append_text(p_str, data);
            if (i != 7)
...
static void
extractSubjectAltName(X509* p_cert, struct mystr* p_str)
{
    ...
    GENERAL_NAMES *subjectAltNames = (GENERAL_NAMES *) X509_get_ext_d2i(p_cert, NID_subject_alt_name, 0, 0);
    if( NULL != subjectAltNames) {
        int numAN = sk_GENERAL_NAME_num(subjectAltNames);
        int length = 0;
        for(int i = 0; i < numAN; ++i ) {
            memset(data, 0, 256);
            GENERAL_NAME *gName = sk_GENERAL_NAME_value(subjectAltNames, i );
            switch ( gName->type ) {
            case GEN_OTHERNAME:
                if (gName->d.otherName->value->type == V_ASN1_UTF8STRING) {
                    length = ASN1_STRING_to_UTF8( &value, gName->d.otherName->value->value.utf8string);
                    if(length > 0) {
                        str_alloc_text(p_str, (const char*)value);
                    }
                    OPENSSL_free(value);
                    value = NULL;
                    i=numAN; // Exit loop
            ...
            case GEN_DIRNAME:
                // Get only Common name, not whole DN
                length = X509_NAME_get_text_by_NID(gName->d.directoryName, NID_commonName, data, sizeof(data));
                str_alloc_text(p_str, data);
                i=numAN; // Exit loop
                break;
            ...
            case GEN_EMAIL:
            case GEN_URI:
                length = ASN1_STRING_to_UTF8(&value, gName->d.ia5 );
                str_alloc_text(p_str, (const char*)value);
                i=numAN; // Exit loop
                OPENSSL_free(value);
                value = NULL;
            ...
            case GEN_IPADD:
                ipStructToStr(gName->d.iPAddress, p_str);
                i=numAN; // Exit loop
                break;

            case GEN_RID:
                length = i2t_ASN1_OBJECT(data, sizeof(data), gName->d.registeredID);
                str_alloc_text(p_str, data);
                i=numAN; // Exit loop
                break;
...
ssl_cert_digest(SSL* p_ssl, struct vsf_session* p_sess, struct mystr* p_str)
{
  ...
  if (tunable_ssl_user_auth_cert)
  {
	  extractSubjectAltName(p_cert, &p_sess->user_str);
  }
