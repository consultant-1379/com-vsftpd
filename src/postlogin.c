...
process_post_login(struct vsf_session* p_sess)
{
  ...
  {
    vsf_banner_dir_changed(p_sess, FTP_LOGINOKNOPWD);
    vsf_cmdio_write(p_sess, FTP_LOGINOKNOPWD, "Login successful without password.");
  } else
  {
  ...
  while(1)
  {
    ...
    {
      vsf_log_start_entry(p_sess, kVSFLogEntryLogout);
...
static void handle_logged_in_user(struct vsf_session* p_sess)
{
  ...
  else if (str_equal(&p_sess->user_str, &p_sess->ftp_arg_str))
  {
	if (p_sess->control_use_ssl && tunable_ssl_user_auth_cert)
    {
      vsf_cmdio_write(p_sess, FTP_LOGINOKNOPWD, "Already logged in.");
    }
    else
    {
