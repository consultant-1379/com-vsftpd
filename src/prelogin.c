...
handle_user_command(struct vsf_session* p_sess)
{
  ...
  if (p_sess->control_use_ssl && tunable_ssl_user_auth_cert)
  {
	  if (str_isempty(&p_sess->user_str))
	  {
		  vsf_cmdio_write(
				  p_sess, FTP_LOGINERR, "The client certificate isn't available, doesn't have subjectAltName extension or the extension value is empty.");
		  return;
	  }
	  if (tunable_ssl_crosscheck_username && !str_equal(&p_sess->ftp_arg_str, &p_sess->user_str))
	  {
		  vsf_cmdio_write(
				  p_sess, FTP_LOGINERR, "The client provided username doesn't match which extracted from certificate.");
		  str_empty(&p_sess->user_str);
		  vsf_log_start_entry(p_sess, kVSFLogEntryLogin);
		  vsf_log_do_log(p_sess, 0);
		  return;
	  }
	  str_copy(&p_sess->ftp_arg_str, &p_sess->user_str);
  } else
  {
  ...
  else
  {
	 if (p_sess->control_use_ssl && tunable_ssl_user_auth_cert)
	 {
		 str_alloc_text(&p_sess->ftp_cmd_str, "PASS");
