...
{
  str_empty(p_str);
  static const char* rdns;
  static int clientport = 0;
  static int serverport = 0;
  static const char* laddr;
  int is_interface_needed = 1;

  if (clientport == 0)
  {
    clientport = vsf_sysutil_sockaddr_get_port(p_sess->p_remote_addr);
  }
  if (serverport == 0)
  {
    serverport = vsf_sysutil_sockaddr_get_port(p_sess->p_local_addr);
  }
  if (!tunable_syslog_enable)
  {
    /* Date - vsf_sysutil_get_current_date updates cached time */
  ...
  switch (what)
  {
    ...
    case kVSFLogEntryDebug:
      ...
      break;
    case kVSFLogEntryLogout:
      str_append_text(p_str, "LOGOUT:");
      is_interface_needed = 0;
      break;
  ...
  if(what != kVSFLogEntryLogout)
  {
    str_append_text(p_str, ": addr=");
    if (!str_isempty(&p_sess->remote_ip_str)){
      str_append_char(p_str, '"');
      str_append_str(p_str, &p_sess->remote_ip_str);
      str_append_char(p_str, '"');
    } else {
  ...
  if(what == kVSFLogEntryLogin)
  {
    str_append_text(p_str, " rport=");
    if(clientport){
      str_append_ulong(p_str,clientport);
    } else {
      str_append_char(p_str, '?');
    }
    laddr = vsf_sysutil_inet_ntop(p_sess->p_local_addr);
    str_append_text(p_str, " laddr=");
    if(laddr){
      str_append_char(p_str, '"');
      str_append_text(p_str, laddr);
      str_append_char(p_str, '"');
    ...
    str_append_text(p_str, " lport=");
    if(serverport){
      str_append_ulong(p_str,serverport);
    } else {
      str_append_char(p_str, '?');
    }
    str_append_text(p_str, " hostname=");
    rdns = vsf_sysutil_getnameinfo(vsf_sysutil_inet_ntop(p_sess->p_remote_addr));
    if(rdns)
    {
      str_append_char(p_str, '"');
      str_append_text(p_str, rdns);
      str_append_char(p_str, '"');
    } else {
      str_append_char(p_str, '?');
    }
  }
  str_append_text(p_str, " ses=");

  /* If XDG_SESSION_ID is present update with that value or append with question mark */

  if ((p_sess->ses != NULL) && (p_sess->ses[0]!='\0')) {
      str_append_text(p_str, p_sess->ses);
  } else {
      str_append_char(p_str, '?');
  }
  ...
  {
    /* Not to display username in case of fail login*/
    if(what == kVSFLogEntryLogin && !succeeded)
    {
      str_append_char(p_str, ' ');
    }
    ...
    {
      str_append_text(p_str, " acct=");
      str_append_str(p_str, &p_sess->user_str);
    }
  }

  /* Interface */
  if(is_interface_needed)
  {
    str_append_text(p_str, " interface=vsftpd");
  }
}
