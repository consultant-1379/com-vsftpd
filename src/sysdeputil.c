                      ...
                      char** ses)
{
  ...
  if (!(control_use_ssl && tunable_ssl_user_auth_cert)) {
  ...
  /* Allocate memory for ses  */
  *ses = malloc(30*sizeof(char));
  char ** pEnv = pam_getenvlist(s_pamh);

  /*Assigning the XDG_SESSION_ID value to ses*/
  if(pEnv!= NULL) {
      while(pEnv[count] != NULL){
          str = malloc((strlen(pEnv[count])+1)*sizeof(char));
          strcpy(str, pEnv[count]);
          char* envVar;
          envVar = strtok(str, "=");
          if(envVar != NULL) {
               if(!strcmp(envVar, "XDG_SESSION_ID")) {
                   sscanf(pEnv[count], "XDG_SESSION_ID=%s", *ses);
                   break;
                }
          }
          free(str);
