...
static void * logger_lib = NULL;

/* LDE Logger Functions */
void (*rfc5424_open)(int log_facility);
void (*rfc5424_close)();
void (*rfc5424_syslog)(char* tag, int priority, int add_pid,
                      char* msgid, const char *format_msg, ...);

void vsf_load_logger_library()
{
  // If security_audit stream is available, this env variable is loaded into the process during start-up.
  // Loading of libraries will only be done in presence of security_audit stream.
  if (getenv("SECURITY_STREAM")) {
    // Step-1 : Load LDE Logger
    logger_lib = dlopen(LDE_LOGGER_LIBRARY, RTLD_LAZY);
    if(logger_lib) {
      rfc5424_open = dlsym(logger_lib,LDE_LOGGER_OPEN_FUNCTION);
      rfc5424_syslog = dlsym(logger_lib,LDE_LOGGER_LOG_FUNCTION);
      rfc5424_close = dlsym(logger_lib,LDE_LOGGER_CLOSE_FUNCTION);

      if(!rfc5424_syslog) {
        syslog(LOG_WARNING,"rfc5424_syslog symbol loading error : %s",dlerror());
        dlclose(logger_lib);
        logger_lib = NULL;
        rfc5424_open = NULL;
        rfc5424_close = NULL;
      ...
      syslog(LOG_WARNING,"liblde-logger.so loading failed with error : %s",dlerror());
    }
...
const char*
vsf_sysutil_getnameinfo(const char* ip)
{
  if(ip == NULL){
    return NULL;
  }
  ...
  //fetching ipv4 if the address is ipv4 embedded ipv6 address.
  char* p = (strstr((char*)ip,"::ffff:"))? strstr(ip,"::ffff:"): strstr((char*)ip,"::FFFF:");
  if(p && p == ip)
  {
    p = p+7;
    ip = p;
  ...
  {
    if (inet_pton(AF_INET6, ip, &(sa.sin_addr)))
    {
...
vsf_sysutil_syslog(const char* p_text, int severe)
{
  severe = LOG_NOTICE;
  if(rfc5424_syslog) {
    rfc5424_syslog(APPLICATION_NAME,severe,1,LOGGER_MESSAGE_ID,"%s",p_text);
  }
}
